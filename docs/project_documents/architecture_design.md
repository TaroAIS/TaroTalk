...
3. **业务服务层**：由多个独立的微服务组成，每个服务完成单一职责，例如 Auth、User、Persona、Chat、Feed、Presence、Notification、Relationship、Scheduler、AI、Orchestrator 等，通过内部 gRPC/REST 调用。
...
1. **用户注册与 persona 生成**：
   ...
   4. Orchestrator 服务启动自举流程，创建 self-agent 与 AI 联系人，调用 User/Contact/Relationship 服务建立关系。
...
2. **发送消息**：
   ...
   4. 若消息由 AI 代理发送，则 Chat Service 调用 Orchestrator 进行多代理编排与工具调用，按相同流程返回。

4. **A2A 群聊与后台剧情**：
   1. Scheduler 服务定时触发 Orchestrator 的 /a2a/simulate。
   2. Orchestrator 根据关系与目标选择代理发言，调用 Chat/Feed/Relationship 等工具接口。
   3. 事件写入存储层并触发 Persona 反思更新。
...
- **Orchestrator Service**：多代理编排与工具调用，负责 A2A 群聊、导演式轮次控制、后台剧情推进。
- **Gateway/Translate Layer**：SDK 封装数据库、中间件等基础能力。

## 数据流与控制流
- **数据流**：消息、动态、事件等数据主要在客户端、Chat Service、Feed Service、存储层之间流动；Orchestrator 会调用 LLM 处理文本并将结果返回业务层。
- **控制流**：用户的操作通过 API 网关路由到对应服务，服务间通过异步消息队列或同步 RPC 调用协调；Scheduler Service 根据定时任务触发 Orchestrator 推动 AI 行为。
